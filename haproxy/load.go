...
func (h Haproxy) GetLoadAsMap(backendName string) ([]map[string]interface{}, error) {
    ...
    load, err := h.GetLoad(backendName)

    if err != nil {
        return nil, err
    }

    loadAsMap := make([]map[string]interface{}, len(load))

    for i, l := range load {


        rlv := reflect.ValueOf(l)
        rlt := reflect.TypeOf(l)
        log.Println(rlt)

        values := make(map[string]interface{})

        for i := 0; i < rlt.Elem().NumField(); i++ {
            value := rlv.Elem().Field(i).Interface()
            values[string(rlt.Elem().Field(i).Tag.Get("csv"))] = value
        }

        loadAsMap[i] = values
    }

    return loadAsMap, nil
}
